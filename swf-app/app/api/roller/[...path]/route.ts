...
const ALLOWED_GET = new Set(['onoff', 'list', 'activelist', 'currentruntime', 'history']);
const ALLOWED_POST = new Set(['updateruntimelimit', 'batchupdateruntimelimit', 'updateruntime', 'replace']);

function joinPath(segments: string[] | undefined): string {
    return (segments ?? []).join('/');
}

function resolveRollerPath(segments: string[] | undefined): { swfPath: string; error?: string } {
    const parts = segments ?? [];
    if (parts.length === 0) {
        return { swfPath: '', error: 'Missing roller API path' };
    }
    ...
    return { swfPath: '', error: `Invalid roller API path: ${parts.join('/')}` };
}

async function handle(req: NextRequest, segments: string[] | undefined) {
    const { swfPath, error: pathError } = resolveRollerPath(segments);
    if (pathError || !swfPath) {
        return NextResponse.json({ error: pathError || 'Missing roller API path' }, { status: 400 });
    }
